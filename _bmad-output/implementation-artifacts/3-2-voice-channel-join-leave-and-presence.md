...
- [ ] Task 1: Install mediasoup-client and add shared types (AC: 2)
  - [ ] 1.1 Install mediasoup-client in client workspace: `npm install mediasoup-client@^3.18.7 -w client`
  - [ ] 1.2 Verify mediasoup-client types are available (ships its own TypeScript types)
  - [ ] 1.3 Add `VoiceChannelPresencePayload` interface to `shared/src/ws-messages.ts` if not already present for voice presence sync (list of `{ userId, channelId }` entries)

- [ ] Task 2: Create mediaService.ts — mediasoup-client Device + Transport management (AC: 2, 3)
  - [ ] 2.1 Create `client/src/renderer/src/services/mediaService.ts`
  - [ ] 2.2 Import `Device` from `mediasoup-client`
  - [ ] 2.3 Implement `initDevice(routerRtpCapabilities)`:
    ```typescript
    const device = new Device()
    await device.load({ routerRtpCapabilities })
    ```
  - [ ] 2.4 Implement `createSendTransport(transportParams, iceServers)`:
    ```typescript
    ...
    ```
  - [ ] 2.5 Wire `transport.on('connect')` — calls `wsClient.request('voice:connect-transport', { transportId, dtlsParameters })`
  - [ ] 2.6 Wire `transport.on('produce')` — calls `wsClient.request('voice:produce', { transportId, kind, rtpParameters })`, returns `{ id: producerId }`
  - [ ] 2.7 Implement `createRecvTransport(transportParams, iceServers)` — same as send but `device.createRecvTransport()`
  - [ ] 2.8 Wire recv `transport.on('connect')` — same pattern as send transport
  - [ ] 2.9 Implement `produceAudio(sendTransport)`:
    ```typescript
    ...
    ```
  - [ ] 2.10 Implement `consumeAudio(recvTransport, { consumerId, producerId, kind, rtpParameters })`:
    ```typescript
    ...
    return consumer
    ```
  - [ ] 2.11 Implement `cleanup()` — close all transports, producers, consumers, stop media tracks
  - [ ] 2.12 Export functions: `initDevice`, `getDevice`, `createSendTransport`, `createRecvTransport`, `produceAudio`, `consumeAudio`, `cleanup`
  - [ ] 2.13 **CRITICAL**: `Device` is a singleton per connection session. Create new Device on each voice join, destroy on leave
  - [ ] 2.14 **CRITICAL**: `transport.on('connect')` and `transport.on('produce')` callbacks MUST call `callback()` on success or `errback(error)` on failure — mediasoup-client will hang if these are not called

- [ ] Task 3: Create useVoiceStore.ts — Zustand voice state management (AC: 1-6)
  - [ ] 3.1 Create `client/src/renderer/src/stores/useVoiceStore.ts`
  - [ ] 3.2 Define store interface:
    ```typescript
    ...
    ```
  - [ ] 3.3 Implement `joinChannel(channelId)`:
  ...
  - [ ] 3.5 Implement `addPeer(channelId, userId)` — add user to `channelParticipants[channelId]`
  - [ ] 3.6 Implement `removePeer(channelId, userId)` — remove user from `channelParticipants[channelId]`
  - [ ] 3.7 Implement `toggleMute()` / `toggleDeafen()` — toggle boolean flags (actual audio mute/deafen is story 3.4, but UI state tracked here)
  - [ ] 3.8 **CRITICAL**: If already in a voice channel when `joinChannel()` is called, leave the current channel first
  - [ ] 3.9 Store follows pattern: `{ isLoading, error, data }` consistent with other stores

- [ ] Task 4: Register voice WebSocket event listeners (AC: 2, 3, 6)
  - [ ] 4.1 In `wsClient.ts`, add handlers for voice broadcast events:
  ...
  - [ ] 4.2 **CRITICAL**: Register handlers AFTER WebSocket connects, before any voice operations
  - [ ] 4.3 Handle `voice:new-producer` flow:
  ...
  - [ ] 4.4 Handle `voice:producer-closed`:
    1. Find consumer by producerId
    2. Close consumer, remove audio element
    3. Clean up references
  - [ ] 4.5 Track active consumers and audio elements in a Map for cleanup on leave

- [ ] Task 5: Create VoiceParticipant.tsx component (AC: 1, 6)
  - [ ] 5.1 Create `client/src/renderer/src/features/voice/VoiceParticipant.tsx`
  - [ ] 5.2 Component props: `{ userId: string }`
  - [ ] 5.3 Render: 24px avatar circle (user initial, colored background from `avatarColor` util) + username
  - [ ] 5.4 Row height: 32px, nested 24px left indent under channel name
  - [ ] 5.5 Look up user display name from `useMemberStore` (already loaded on app init)
  - [ ] 5.6 Muted indicator: small `MicOff` icon overlay on avatar (when user is muted) — **deferred to story 3.4 (mute state not tracked yet)**
  - [ ] 5.7 Speaking indicator: green ring (`#23a55a`) around avatar — **deferred to story 3.3 (voice activity detection not yet implemented)**
  - [ ] 5.8 For now, just render avatar + username (mute overlay + speaking ring added in later stories)

- [ ] Task 6: Create VoiceStatusBar.tsx component (AC: 2, 3, 5)
  - [ ] 6.1 Create `client/src/renderer/src/features/voice/VoiceStatusBar.tsx`
  - [ ] 6.2 Only render when `useVoiceStore.currentChannelId !== null`
  - [ ] 6.3 Layout: 52px height, full sidebar width (240px), 12px internal padding, `bg-tertiary` background, `border-t border-bg-hover`
  - [ ] 6.4 Anatomy (top to bottom, left to right):
  ...
  - [ ] 6.5 Button styles: no background by default, `text-muted` color, hover `bg-hover`, active/toggled shows `accent-primary` or crossed icon
  - [ ] 6.6 Disconnect button: always red background (`bg-error`), white icon
  - [ ] 6.7 **Subtle slide-up animation on appear** (if `prefers-reduced-motion` not set)
  - [ ] 6.8 ARIA labels: "Mute microphone", "Deafen audio", "Toggle video", "Disconnect from voice"
  - [ ] 6.9 Icons from `lucide-react`: `Mic`, `MicOff`, `Headphones`, `HeadphoneOff`, `Video`, `VideoOff`, `PhoneOff`

- [ ] Task 7: Update ChannelSidebar to show voice participants (AC: 1, 6)
  - [ ] 7.1 In `client/src/renderer/src/features/channels/ChannelSidebar.tsx`:
  ...
  - [ ] 7.2 In `client/src/renderer/src/features/channels/ChannelItem.tsx`:
  ...
  - [ ] 7.3 Voice channel expansion: channels expand to show nested participants when anyone is connected
  - [ ] 7.4 Voice participant list renders below the channel name with 24px left indent
  - [ ] 7.5 Insert VoiceStatusBar in the sidebar layout between channel list and UserPanel:
    ```
...
- [ ] Task 8: Add connect/disconnect audio cues (AC: 2, 3)
  - [ ] 8.1 Create or source two short audio files:
  ...
  - [ ] 8.2 Create `client/src/renderer/src/utils/soundPlayer.ts`:
    ```typescript
    ...
    ```
  - [ ] 8.3 Call `playConnectSound()` after successful voice join in `useVoiceStore.joinChannel()`
  - [ ] 8.4 Call `playDisconnectSound()` after voice leave in `useVoiceStore.leaveChannel()`
  - [ ] 8.5 **NOTE**: If sourcing audio is complex, use Web Audio API to generate simple tones programmatically (ascending beep for connect, descending for disconnect) — no external files needed

- [ ] Task 9: Keyboard shortcuts for voice controls (AC: 5)
  - [ ] 9.1 Register global keyboard shortcuts (in AppLayout or a dedicated hook):
  ...
  - [ ] 9.2 Only active when `currentChannelId !== null`
  - [ ] 9.3 Use `useEffect` with `keydown` event listener, check `metaKey || ctrlKey` + `shiftKey` + key

- [ ] Task 10: Handle voice cleanup on WebSocket disconnect (AC: 2, 3)
  - [ ] 10.1 When WebSocket disconnects (connection lost), auto-cleanup voice state:
  ...
  - [ ] 10.2 **Do NOT auto-reconnect voice** — per project context, WebRTC has no auto-reconnect; user manually rejoins
  - [ ] 10.3 On WebSocket reconnect, request a fresh `voice:presence-sync` from server or rebuild `channelParticipants` from server state
  - [ ] 10.4 In wsClient disconnect handler, call `useVoiceStore.getState().leaveChannel()` (but skip the `voice:leave` WS request since connection is already down — just do local cleanup)

- [ ] Task 11: Write tests (AC: 1-6)
  - [ ] 11.1 Create `client/src/renderer/src/stores/useVoiceStore.test.ts`:
  ...
  - [ ] 11.2 Create `client/src/renderer/src/features/voice/VoiceStatusBar.test.tsx`:
  ...
  - [ ] 11.3 Create `client/src/renderer/src/features/voice/VoiceParticipant.test.tsx`:
    - Test renders avatar with user initial and colored background
    - Test renders username
    - Test correct styling (32px height, 24px indent)
  - [ ] 11.4 Update `client/src/renderer/src/features/channels/ChannelSidebar.test.tsx` (if exists):
  ...
  - [ ] 11.5 Create `client/src/renderer/src/services/mediaService.test.ts`:
...
- [ ] Task 12: Final verification (AC: 1-6)
  - [ ] 12.1 Run `npm test -w client` — all new + existing tests pass
  - [ ] 12.2 Run `npm run lint` — no lint errors
  - [ ] 12.3 Run `npm run build -w client` — builds successfully with mediasoup-client
  - [ ] 12.4 Manual test: start client + server, click voice channel → voice status bar appears, "Connecting..." then "Voice Connected"
  - [ ] 12.5 Manual test: other user sees your name in voice channel participant list
  - [ ] 12.6 Manual test: click disconnect → status bar disappears, name removed from participant list
  - [ ] 12.7 Manual test: navigate text channels while in voice → voice persists
  - [ ] 12.8 Manual test: connect/disconnect sounds play
  - [ ] 12.9 Manual test: keyboard shortcuts work (Ctrl+Shift+M/D/E)
